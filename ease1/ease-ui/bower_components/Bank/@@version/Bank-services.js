define(["angular"], function(angular) {
  ...
  BankAccountDetailsFactory.$inject = [
    "EaseConstant",
    "EaseConstantFactory",
    "EASEUtilsFactory",
    "$q",
  ...
  function BankAccountDetailsFactory(
    EaseConstant,
    EaseConstantFactory,
    EASEUtilsFactory,
    $q,
    ...
    $rootScope.$on("$viewContentLoading", function() {
      if (!$state.$current.locals) $state.$current.locals = {};
    });
    ...
    function getAvailableCardActions(accountDetailsRefId) {
      var url = "Bank/accounts/" +
        encodeURIComponent(accountDetailsRefId) +
        "/features";
    ...
    function getBankAccountDetailsFromOL(
      accountDetailsRefId,
      productType,
      productId
    ) {
      ...
      var urlPrefix = productType === EaseConstant.lineOfBusiness.cd
        ? EaseConstant.urlPrefixerCDDeposits
        : EaseConstant.urlPrefixerDeposits;
  ...
  function BankLocalization(BankFiles, $q, $locale, $http) {
    ...
    function getBundle() {
      var deferred = $q.defer();
      var endpoint = BankFiles.getFilePath(
        "utils/i18n/resources-locale_en-us.json"
      );
      var callForResourceBundle = $http.get(endpoint);

      callForResourceBundle.then(
