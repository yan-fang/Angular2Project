define([
  ...
  function DebitOtpController(
    ...
    var selectDefaultContactPoint = function(contactPoints) {
      var phones = contactPoints.sms.map(function(phoneNumber) {
        return {type: 'sms', value: phoneNumber}
      });
      var emails = contactPoints.email.map(function(emailAddress) {
    ...
    vm.startOtp = function(businessEventId) {
      return DebitLocalization.get()
        .then(function(i18n) {
          ...
          if (contactPoints.data.email.length ||
              contactPoints.data.sms.length) {
